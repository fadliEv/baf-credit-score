...
type Credit struct {
	...
	ProductType      string   `gorm:"type:varchar(50);not null;check:status IN ('CAR','MOTOR','FURNITURE')"`       // Jenis produk kredit
	LoanAmount       float64  `gorm:"not null"`                                                                    // Jumlah pinjaman
	Tenure           int      `gorm:"not null"`                                                                    // Lama tenor dalam bulan
	EmploymentStatus string   `gorm:"type:varchar(20);not null;check:status IN ('PKWT','PKWTT')"`                  // Status pekerjaan customer
